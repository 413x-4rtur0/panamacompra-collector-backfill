...
## Unified records
- **Single path:** `/mnt/pcc-data/panamacompra-unified-records` (records + data). Both hosts mount the same external `pcc-data` drive (`hp-15` local, `hp-23` via `sshfs a2gutierrezmora@hp-15-bw036nr-d.local:/media/.../pcc-data /mnt/pcc-data` with `allow_other,x-systemd.automount`). Legacy `/mnt/pcc-data-hp15` kept as fallback until migration finishes.
