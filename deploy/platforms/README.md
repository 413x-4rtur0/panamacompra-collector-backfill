...
|----------|------|----------------|-------------|------------|
| `amd64-mint` | hp-23-g201la-l (192.168.10.20) | x86_64 | Native (`./setup.sh`, Firefox host) | `/mnt/pcc-data` primary, `/mnt/pcc-data` legacy fallback |
| `i386-debian-amd64-container` | hp-15-bw036nr-d (192.168.10.40) | i386 (32-bit) | `docker` `debian:bookworm-slim --platform linux/amd64 --security-opt seccomp=unconfined` (Firefox inside container) | Same `/mnt/pcc-data` (local external `pcc-data` drive) |
...
## Unified records
- **Single path:** `/mnt/pcc-data/panamacompra-unified-records` (records + data). Both hosts mount the same external `pcc-data` drive (`hp-15` local, `hp-23` via `sshfs a2gutierrezmora@hp-15-bw036nr-d.local:/media/.../pcc-data /mnt/pcc-data` with `allow_other,x-systemd.automount`). Legacy `/mnt/pcc-data` kept as fallback until migration finishes.
