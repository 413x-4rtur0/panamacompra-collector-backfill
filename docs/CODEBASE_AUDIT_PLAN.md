...
| **3** | Delivery tracking: additive `notify_attempts`/`notify_error` columns via the migration map (`src/common.py:1166`); Failed-alerts card; chart→Records drilldown; CSV export of the filtered slice | `src/common.py`, `src/pipeline/020-notify-whatsapp.py`, monitors | Medium (additive schema) | migration on a DB copy; `pcc kpi`; `070-test-zone.py --limit 2 --apply` | revert code; new columns stay inert |
| **4** | Shared monitor library: extract the 11 duplicated helpers into `src/monitor/monitor_common.py`; add a `--json` mode; retire the `bin/pcc` KPI heredoc | new `src/monitor/monitor_common.py`, `001a`, `001b`, `bin/pcc` | Medium | `py_compile` all three; **number-parity check** web vs tk vs `pcc kpi` | revert; no data touched |
| **5** | Automation/health hardening: systemd unit type fix (`Type=oneshot`+`RemainAfterExit` or a watcher unit); dev-mode guard for `git reset --hard`; random WAHA dashboard password at setup; optional freshness alarm (WAHA `system` purpose when no successful run in N hours) | `systemd/user/*.service`, `src/pipeline/000-update-before-run.sh`, `setup.sh`, `src/tools/010-docker-stack.sh` | Medium | `systemctl --user daemon-reload && start panamacompra`; `pcc webhook diag`; `review-system.sh` | keep `.bak` unit files during transition; revert commit |
