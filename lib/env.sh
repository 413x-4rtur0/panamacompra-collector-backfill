...
if [ -d /mnt/pcc-data ] && [ ! -d /mnt/pcc-data-hp15 ]; then
  ...
  if [[ "$PC_DATA_DIR" == *"pcc-data-hp15"* ]]; then
    ...
    export PC_RECORDS_DIR="${PC_RECORDS_DIR/pcc-data-hp15/pcc-data}"
  fi
fi
# Fresh default case for new hosts (no PC_DATA_DIR yet)
if [ -z "${PC_DATA_DIR:-}" ] || [ "$PC_DATA_DIR" = "$PC_STATE_DIR/data" ]; then
