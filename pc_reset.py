...
is passed, so an accidental click cannot erase the archive.
    ...
    ./pc_reset.py requeue-details   # re-download every detail page next run
    ./pc_reset.py reset-notify      # re-announce every record from zero (WAHA)
    ./pc_reset.py wipe-db    --yes  # delete the tracking DB (keeps record files)
    ./pc_reset.py wipe-all   --yes  # delete DB + every downloaded record/calendar
