...
```bash
./bin/pcc launcher install          # create/update menu + desktop shortcut
./bin/pcc launcher install --no-desktop
./bin/pcc launcher remove
./bin/pcc launcher path
```

For operators who prefer filenames to show workflow order, `scripts/tasks/` contains ordered wrapper names such as `000-review-legacy-and-new-names.sh`, `001a-setup-development.sh`, `001b-install-update-monitor-launcher.sh`, `002-install-user-services.sh`, `020-start-collector.sh`, and `090-uninstall-or-purge.sh`; run `./bin/pcc review-names`, run `./bin/pcc doctor` before a from-zero reinstall, run `./bin/pcc self-contained` before Docker/compose deployment, use `./bin/pcc data export`/`import` to move downloaded records into a new install, use the full sorted alias surface in `scripts/ordered/`, or see `scripts/README.md` for the naming methodology.
