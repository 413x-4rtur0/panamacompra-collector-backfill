...
4. **Settings (editable)** — entry fields pre-filled with the current values; change what you need and leave the rest, then click **Apply & save settings**:
   ...
   - WhatsApp source label, default destination chat id, per-purpose chat ids (index alerts / item details / status + system health, each optional), and keyword filter.
