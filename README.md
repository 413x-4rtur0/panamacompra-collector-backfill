...
Transparency, refresh cadence, WhatsApp/calendar toggles and the auto-close countdown can all be changed from the Settings panel without restarting the monitor. The web monitor (`pc_monitor_server.py`) exposes the same ETA, toggles, multi-select record-index actions and `/api/record-index` endpoint.
