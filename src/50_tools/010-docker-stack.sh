...
if command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1; then
  ...
  fail "Docker (with the compose plugin) is not installed. Install it and rerun: ./src/50_tools/010-docker-stack.sh $ACTION"
fi
...
preflight() {
  ...
  for legacy_app in "${HOME:-/root}/Apps/waha" "/Apps/waha" "${HOME:-/root}/Apps/panamacompra-monitor" "/Apps/panamacompra-monitor" "${HOME:-/root}/Apps/panamacompra-webhook-receiver" "/Apps/panamacompra-webhook-receiver"; do
    if [ -d "$legacy_app" ] && [ ! -L "$legacy_app" ]; then
      note "Found previous installation data at $legacy_app — consolidate it into this checkout with ./src/50_tools/100-migrate-apps-layout.sh (dry-run by default)."
    fi
  done
}
