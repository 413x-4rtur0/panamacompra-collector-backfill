...
#   PC_GITHUB_USER=myuser PC_GITHUB_TOKEN=ghp_xxx ./src/50_tools/120-setup-git-credentials.sh
...
if [ -n "$gh_token" ]; then
  ...
  echo "  • write it once:  echo <TOKEN> > $TOKEN_FILE  &&  ./src/50_tools/120-setup-git-credentials.sh"
