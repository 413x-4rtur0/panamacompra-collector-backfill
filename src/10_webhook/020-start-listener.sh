...
Starts src/10_webhook/010-webhook-listener.py in the background.
...
print_port_owner_hint() {
  echo "Port $PORT is already in use, but no local src/10_webhook/010-webhook-listener.py process was detected." >&2
...
LISTENER="$APP_ROOT/src/10_webhook/010-webhook-listener.py"
