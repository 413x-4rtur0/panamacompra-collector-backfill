...
CHANGEDETECTION_BROWSER_STEPS_CLOSED_JS = pc_common.APP_ROOT / "config" / "changedetection-browser-steps-closed.js"
...
def location_keyword_filter_fn(query: str):
    ...
    of ones that merely mention it in passing. A location that's written into
    the free-text description instead of a structured field is still
...
VALUE_SETTING_DEFAULTS = {
    ...
    # Matches 020-notify-whatsapp.py's own coded default (index_digest_threshold(),
    # default 1) — must stay in sync or a settings reset silently reintroduces
    # one-message-per-opportunity instead of grouped digests.
    "PC_NOTIFY_INDEX_DIGEST_THRESHOLD": "1",
    ...
    # Optional targeted date-range backfill (YYYY-MM-DD, blank = day-count
    # default via PC_CLOSED_BACKFILL_DAYS). See 037-collect-closed-index.py.
    "PC_CLOSED_BACKFILL_START_DATE": "",
    "PC_CLOSED_BACKFILL_END_DATE": "",
...
MANUAL_ACTIONS = [
    ...
    ManualAction("Closed", "Run Closed backfill now", ("./src/20_pipeline/039-run-closed-backfill.sh",), "Runs one historical-backfill segment immediately instead of waiting for the 20-minute timer. Safe to click anytime: it skips cleanly (logs why, does nothing else) if the main worker or the Closed new-closures run is already active."),
    ManualAction("Closed", "Pause Closed backfill timer", ("systemctl", "--user", "stop", "panamacompra-closed-backfill.timer"), "Stops the recurring 20-minute backfill timer until resumed below. New-closures detection (the separate priority-2 webhook route) and the main pipeline are unaffected — this only pauses the historical backfill segments."),
    ManualAction("Closed", "Resume Closed backfill timer", ("systemctl", "--user", "start", "panamacompra-closed-backfill.timer"), "Restarts the 20-minute backfill timer after a pause. The cursor resumes from wherever it left off; no progress is lost by pausing."),
    ManualAction("Closed", "Stop Closed backfill", ("./src/20_pipeline/120c-stop-closed-backfill.sh",), "Kills an in-progress backfill segment immediately. Only targets the backfill's own process tree (via its lock file), so a Closed new-closures run using the same detail/cotizacion scripts is never touched. Does not pause the timer — the next scheduled tick still starts a fresh segment unless you also pause it above."),
    ...
    ManualAction("Settings", "Repair failed records", ("./src/20_pipeline/125-run-priority.sh", "repair", "90", "repair", "--", "./src/20_pipeline/060b-repair-failed.sh"), "Re-downloads every record stuck at detail_status='failed', any group. Safe to click anytime: defers to priority 1/2/3 (skips cleanly and logs why if any is active) the same way the Closed backfill button does. See progress below."),
...
def closed_webhook_access_payload() -> dict[str, object]:
    """Same shape as webhook_access_payload(), for the independent Closed
    new-closures route (arl89/v1-style separate token) — its own
    changedetection watch, its own trigger chain
    (src/10_webhook/070-run-collector-closed-new.sh), never the full
    pipeline. Read fresh on every call for the same reason."""
    token_path = BASE_DIR / ".webhook_token_closed"
    try:
        token = token_path.read_text(encoding="utf-8").strip() if token_path.exists() else ""
    except OSError:
        token = ""
    ...
    return {
        ...
        "token_file": str(token_path),
        "port": port,
        "changedetection_url": f"json://{public_host}:{port}/panamacompra-closed/{shown}{query}",
        "compose_url": f"json://webhook:8765/panamacompra-closed/{shown}{query}",
        "docker_to_host_url": f"http://{public_host}:{port}/panamacompra-closed/{shown}",
        "local_url": f"http://127.0.0.1:{port}/panamacompra-closed/{shown}",
        "generated_by": "one-time: python3 -c \"import secrets; print(secrets.token_urlsafe(32))\" > .webhook_token_closed (then restart panamacompra-webhook.service)",
    }


def closed_news_sample_pages_current() -> int:
    """The changedetection watch's own page-sampling cap (maxPagesSafety in
    config/changedetection-browser-steps-closed.js) — read straight from the
    local script file, which is the source of truth 135-apply-closed-news-
    pages.py writes to, rather than a separate setting that could drift from
    what the live watch actually has baked in."""
    try:
        text = CHANGEDETECTION_BROWSER_STEPS_CLOSED_JS.read_text(encoding="utf-8")
        match = re.search(r"maxPagesSafety:\s*(\d+),", text)
        return int(match.group(1)) if match else 0
    except OSError:
        return 0


def closed_status_payload() -> dict[str, object]:
    """Live status for the two independent Closed jobs (priority 2:
    ...
    payload: dict[str, object] = {
        ...
        "backfill_days_target": pc_common.env_int("PC_CLOSED_BACKFILL_DAYS", "365", minimum=1),
        "last_forward_run_at": "",
        "last_backfill_run_at": "",
        # parse_settings_file(), not pc_common.env_int(): this long-running
        # process's own os.environ never gets the settings file re-sourced
        # into it (only the short-lived collector scripts, invoked via bash
        # wrappers that do `set -a; . monitor_settings.env`, see that) — the
        # env_int() path would always show the hardcoded default here.
        "forward_page_cap": int(parse_settings_file().get("PC_CLOSED_FORWARD_PAGES") or "20"),
        "news_sample_pages": closed_news_sample_pages_current(),
    }
    try:
        conn = sqlite3.connect(f"file:{ARCHIVE_DB}?mode=ro", uri=True, timeout=2)
        conn.row_factory = sqlite3.Row
        try:
            ...
            state = conn.execute("SELECT * FROM closed_crawl_state WHERE id = 1").fetchone()
            if state:
...
def repair_status_payload() -> dict[str, object]:
    """F11 repair mode: plain read of repair_progress.env (written by
    060-repair-failed.py) plus whether the worker process/flags are
    currently live, same shape as the other status payloads here."""
    progress_path = pc_common.LOG_DIR / "repair_progress.env"
    fields: dict[str, str] = {}
    ...
    return {
        "progress": fields,
        "in_progress": (pc_common.QUEUE_DIR / "repair_in_progress.flag").exists(),
        "requested": (pc_common.QUEUE_DIR / "repair_requested.flag").exists(),
        "running": running("[0]60-repair-failed.py"),
    }
...
<div class="card" data-tab="operations"><h2>Repair status <span class="small">(F11 — see Repair failed records button above)</span></h2><p class="small">Re-downloads every record stuck at detail_status='failed', any group, reusing the same watchdog-protected fetch as the normal collectors. Defers to priority 1/2/3, so it may show "waiting" for a while if any of those are active.</p><pre id="repair-status">Loading…</pre><p><button onclick="loadRepairStatus()">Refresh repair status</button></p></div>
...
const ZONE_DESCRIPTIONS = {{
  ...
  'Closed': 'Manual control over the historical price/provider backfill (runs automatically every 20 min otherwise).',
}};
function renderActionZones() {{
  const root = document.getElementById('action-zones');
  const zones = [...new Set(actionZones.map(a => a.zone))];
  root.innerHTML = zones.map(zone => `<div class="zone"><h3>${{esc(t(zone))}}</h3><div class="zone-desc small">${{esc(t(ZONE_DESCRIPTIONS[zone] || ''))}}</div>` + actionZones.filter(a => a.zone === zone).map(a => `<button onclick="runAction('${{esc(a.label)}}')">${{esc(t(a.label))}}</button><span class="small">${{esc(t(a.comment))}}</span><br>`).join('') + `</div>`).join('');
}}
...
moveWebhookAutoRunControlToScheduler();
function saveMonitorSetting(key, value) {{ return postForm('/api/monitor-setting', 'key=' + encodeURIComponent(key) + '&value=' + encodeURIComponent(value)); }}
...
function saveAdvancedSettings() {{
  ...
  ['PC_WAHA_SOURCE','PC_AUTORUN_SOURCE','PC_CRON_INDEX_LIMIT','PC_CRON_DETAIL_LIMIT','PC_MONITOR_HOST','PC_NEXT_RUN_INTERVAL_MINUTES','PC_MONITOR_DEADLINE_SOON_DAYS','PC_WEBHOOK_INDEX_LIMIT','PC_WEBHOOK_DETAIL_LIMIT','PC_NOTIFY_WITHIN_DAYS','PC_WAHA_RETRIES','PC_WAHA_SEND_DELAY_SECONDS','PC_NOTIFY_INDEX_DIGEST_THRESHOLD','PC_NOTIFY_IDLE_EVERY_HOURS','PC_WAHA_BASE_URL','PC_WAHA_SESSION','PC_WAHA_NOTIFY_EVENTS','PC_TEST_ZONE_LIMIT','PC_MONITOR_STALE_SECONDS','PC_NEXT_RUN_TIMER_WIDTH','PC_NEXT_RUN_TIMER_HEIGHT','PC_NEXT_RUN_TIMER_TOP','PC_NEXT_RUN_TIMER_RECORDS','PC_NEXT_RUN_TIMER_DATA_REFRESH_SECONDS','CHANGEDETECTION_BASE_URL','PC_WEBHOOK_PORT','PC_WEBHOOK_PUBLIC_HOST','WAHA_PORT','WAHA_API_KEY','WAHA_DASHBOARD_USERNAME','WAHA_DASHBOARD_PASSWORD','PC_FIREBASE_WEB_API_KEY','PC_FIREBASE_WEB_AUTH_DOMAIN','PC_FIREBASE_WEB_PROJECT_ID','PC_FIREBASE_WEB_APP_ID','PC_ADMIN_USERNAME','PC_ADMIN_PASSWORD','PC_ADMIN_EMAILS','PC_ADMIN_API_TOKEN','PC_ARL89_FEED_TOKEN','PC_CLOSED_FORWARD_PAGES'].forEach(key => {{ const el = document.getElementById('set-' + key); if (el) saveMonitorSetting(key, el.value); }});
}}
async function saveClosedDateRange() {{
  if (!monitorSettingsLoaded) {{ alert('Settings are still loading — try again in a moment.'); return; }}
  // Sequential, not fired in parallel: /api/monitor-setting does a
  // read-modify-write of the whole settings file, so two concurrent saves
  // can race and one key silently loses its update.
  for (const key of ['PC_CLOSED_BACKFILL_START_DATE', 'PC_CLOSED_BACKFILL_END_DATE']) {{
    const el = document.getElementById('set-' + key);
    if (el) await saveMonitorSetting(key, el.value);
  }}
}}
...
const STATIC_TRANSLATIONS = {{
  es: {{
    ...
    '(read-only)': '(solo lectura)', 'Record selector and filters': 'Selector y filtros de registros', 'Legend:': 'Leyenda:', 'upcoming': 'próximos', 'next to expire': 'próximos a vencer', 'expired': 'vencidos', 'Records Pendings': 'Registros pendientes', 'Records Pendings: —': 'Registros pendientes: —', 'Records Completed': 'Registros completados', 'Records Completed: —': 'Registros completados: —', 'Read-only archive database summary with counters, status breakdown, recent records and DB elements/columns.': 'Resumen de la base de datos de archivo, solo lectura, con contadores, desglose de estado, registros recientes y elementos/columnas de la BD.', 'Refresh DB summary': 'Actualizar resumen de la BD', 'Same database details in a collapsible review panel. Refresh after a run or a reset.': 'Los mismos detalles de la base de datos en un panel de revisión colapsable. Actualiza después de una ejecución o un reinicio.', 'Refresh DB snapshot': 'Actualizar instantánea de la BD', 'Database review': 'Revisión de la base de datos', 'Database summary': 'Resumen de la base de datos', 'Use Record selector and filters → Detail status = Pending records for full selectors/open actions.': 'Usa Selector y filtros de registros → Estado del detalle = Registros pendientes para selectores/acciones completos.', 'Use Record selector and filters → Detail status = Completed records for full selectors/open actions.': 'Usa Selector y filtros de registros → Estado del detalle = Registros completados para selectores/acciones completos.', 'Closed quotations (cuadro de cotizaciones)': 'Cotizaciones de Cerradas (cuadro de cotizaciones)', 'Item prices and providers collected from the quotation comparison table of each closed opportunity. Search by opportunity number and/or an item keyword; each result row is one item across every bidder, with the winning (lowest) price highlighted.': 'Precios de ítems y proveedores recolectados del cuadro de cotizaciones de cada oportunidad cerrada. Busca por número de oportunidad y/o palabra clave de ítem; cada fila de resultado es un ítem entre todos los proponentes, con el precio ganador (más bajo) resaltado.', 'Opportunity number': 'Número de oportunidad', 'Item search': 'Búsqueda de ítem', 'Search by opportunity number or item to see prices per provider.': 'Busca por número de oportunidad o ítem para ver precios por proveedor.', 'Searching…': 'Buscando…', 'Could not load quotations: ': 'No se pudieron cargar las cotizaciones: ', 'No quotations matched — try a different opportunity number or item keyword.': 'No hay cotizaciones que coincidan — prueba otro número de oportunidad o palabra clave de ítem.', 'Opportunity': 'Oportunidad', 'Item': 'Ítem', 'Bidders': 'Proponentes', 'Min price': 'Precio mín.', 'Avg price': 'Precio prom.', 'Max price': 'Precio máx.', 'Best provider': 'Mejor proveedor', 'View full cuadro': 'Ver cuadro completo', 'Loading full cuadro…': 'Cargando cuadro completo…', 'Could not load the full cuadro: ': 'No se pudo cargar el cuadro completo: ', 'No bid detail stored for this opportunity.': 'No hay detalle de cotizaciones guardado para esta oportunidad.', 'Provider': 'Proveedor', 'Unit price': 'Precio unitario', 'Quoted qty.': 'Cant. cotizada', 'Net amount': 'Monto neto', 'Full cuadro': 'Cuadro completo',
    "Start, queue or stop collection runs.": "Iniciar, encolar o detener ejecuciones de recolección.", "Docker stack, changedetection and WAHA dashboards.": "Pila Docker, paneles de changedetection y WAHA.", "Diagnostics, reports and sandbox runs.": "Diagnósticos, informes y ejecuciones de prueba.", "Maintenance, repair and configuration helpers.": "Ayudas de mantenimiento, reparación y configuración.", "Manual control over the historical price/provider backfill (runs automatically every 20 min otherwise).": "Control manual sobre el relleno histórico de precios/proveedores (de lo contrario se ejecuta automáticamente cada 20 min).", "Run full collector": "Ejecutar recolector completo", "Run collector now": "Ejecutar recolector ahora", "STOP all runners": "DETENER todos los ejecutores", "START all infrastructure": "INICIAR toda la infraestructura", "Pause for development": "Pausar para desarrollo", "Resume automatic collection": "Reanudar recolección automática", "Show run status": "Mostrar estado de ejecución", "Run Closed backfill now": "Ejecutar relleno de Cerradas ahora", "Pause Closed backfill timer": "Pausar temporizador de relleno de Cerradas", "Resume Closed backfill timer": "Reanudar temporizador de relleno de Cerradas", "Stop Closed backfill": "Detener relleno de Cerradas", "Test zone": "Zona de prueba", "Review system": "Revisar sistema", "Full diagnostic report": "Informe de diagnóstico completo", "Update local copy": "Actualizar copia local", "Pre-run update only": "Solo actualización previa a la ejecución", "Upload local changes to GitHub": "Subir cambios locales a GitHub", "Rename folders": "Renombrar carpetas", "Migrate records": "Migrar registros", "Start/refresh docker stack": "Iniciar/actualizar pila Docker", "Docker stack status": "Estado de la pila Docker", "Restart docker stack": "Reiniciar pila Docker", "Stop docker stack": "Detener pila Docker", "Apply work templates": "Aplicar plantillas de trabajo", "Build detail views": "Generar vistas de detalle", "Repair missing deadlines": "Reparar fechas límite faltantes", "Repair failed records": "Reparar registros fallidos", "Build calendars": "Generar calendarios", "Webhook listener": "Listener de webhook", "Install webhook service": "Instalar servicio de webhook", "Open web monitor": "Abrir monitor web", "Open changedetection app window": "Abrir ventana de la app de changedetection", "Print changedetection JS setup": "Imprimir configuración JS de changedetection", "Open WAHA app window": "Abrir ventana de la app de WAHA", "Queues a manual restart run for all available index pages and opens/reuses this monitor.": "Encola una ejecución manual de reinicio para todas las páginas de índice disponibles y abre/reutiliza este monitor.", "Starts the run-all worker immediately for all available index pages and unlimited detail pages.": "Inicia el worker de ejecución completa de inmediato para todas las páginas de índice disponibles y detalles ilimitados.", "Stops the active collection (worker/index/detail/test/calendar) and prevents auto-resume. The monitor, next-run timer and webhook stay running.": "Detiene la recolección activa (worker/índice/detalle/prueba/calendario) y evita la reanudación automática. El monitor, el temporizador de próxima ejecución y el webhook siguen funcionando.", "DANGER: stops workers, test zone, calendar builder, webhook listener and updaters. All monitors stay open so you can resume from here.": "PELIGRO: detiene workers, zona de prueba, generador de calendarios, listener de webhook y actualizadores. Todos los monitores permanecen abiertos para que puedas reanudar desde aquí.", "Counterpart to STOP all runners: brings Docker integrations (changedetection/WAHA/sockpuppetbrowser) and the webhook listener back up, and opens the monitor. Does not queue a collector run by itself.": "Contraparte de DETENER todos los ejecutores: vuelve a levantar las integraciones Docker (changedetection/WAHA/sockpuppetbrowser) y el listener de webhook, y abre el monitor. No encola una ejecución del recolector por sí sola.", "Stops any active run and pauses webhook/cron auto-triggers plus the updater's autostash, so editing this repo is safe. Docker integrations and the monitors stay running.": "Detiene cualquier ejecución activa y pausa los disparadores automáticos de webhook/cron además del autostash del actualizador, para que editar este repositorio sea seguro. Las integraciones Docker y los monitores siguen funcionando.", "Restores every setting 'Pause for development' changed, to its exact previous value. Does not queue a run by itself.": "Restaura cada configuración que cambió ‘Pausar para desarrollo’ a su valor exacto anterior. No encola una ejecución por sí sola.", "Writes a process/log status snapshot to the manual action log.": "Escribe una instantánea del estado de procesos/registro en el registro de acciones manuales.", "Runs one historical-backfill segment immediately instead of waiting for the 20-minute timer. Safe to click anytime: it skips cleanly (logs why, does nothing else) if the main worker or the Closed new-closures run is already active.": "Ejecuta un segmento de relleno histórico de inmediato en lugar de esperar el temporizador de 20 minutos. Seguro de usar en cualquier momento: se omite limpiamente (registra por qué, no hace nada más) si el worker principal o la ejecución de nuevos cierres de Cerradas ya está activa.", "Stops the recurring 20-minute backfill timer until resumed below. New-closures detection (the separate priority-2 webhook route) and the main pipeline are unaffected — this only pauses the historical backfill segments.": "Detiene el temporizador recurrente de relleno de 20 minutos hasta que se reanude abajo. La detección de nuevos cierres (la ruta de webhook independiente de prioridad 2) y el flujo principal no se ven afectados — esto solo pausa los segmentos de relleno histórico.", "Restarts the 20-minute backfill timer after a pause. The cursor resumes from wherever it left off; no progress is lost by pausing.": "Reinicia el temporizador de relleno de 20 minutos después de una pausa. El cursor continúa desde donde quedó; no se pierde progreso al pausar.", "Kills an in-progress backfill segment immediately. Only targets the backfill's own process tree (via its lock file), so a Closed new-closures run using the same detail/cotizacion scripts is never touched. Does not pause the timer — the next scheduled tick still starts a fresh segment unless you also pause it above.": "Detiene de inmediato un segmento de relleno en curso. Solo afecta al árbol de procesos propio del relleno (mediante su archivo de bloqueo), por lo que una ejecución de nuevos cierres de Cerradas que use los mismos scripts de detalle/cotización nunca se ve afectada. No pausa el temporizador — el próximo ciclo programado igual inicia un segmento nuevo a menos que también lo pauses arriba.", "Queues the isolated test behind active work, then opens that sandbox folder.": "Encola la prueba aislada detrás del trabajo activo y luego abre esa carpeta de pruebas.", "Runs the repository health review and troubleshooting summary; on completion WAHA sends a System health message to the system destination (override with pcc health --chat-id/--purpose).": "Ejecuta la revisión de salud del repositorio y el resumen de solución de problemas; al finalizar, WAHA envía un mensaje de salud del sistema al destino del sistema (anula con pcc health --chat-id/--purpose).", "Creates a complete Markdown diagnostic report covering paths, settings, tools, integrations, queues, database counters, processes and recent logs.": "Crea un informe de diagnóstico completo en Markdown que cubre rutas, configuración, herramientas, integraciones, colas, contadores de base de datos, procesos y registros recientes.", "Runs the updater at the highest priority after active work finishes, then reopens the monitor.": "Ejecuta el actualizador con la máxima prioridad después de que termine el trabajo activo, y luego reabre el monitor.", "Queues the lightweight git/dependency refresh ahead of automatic collection.": "Encola la actualización ligera de git/dependencias antes de la recolección automática.", "Commits local checkout changes and pushes the current branch to GitHub/origin before other machines update.": "Confirma los cambios locales del checkout y envía la rama actual a GitHub/origin antes de que otras máquinas se actualicen.", "Queues folder normalization so it cannot overlap collection.": "Encola la normalización de carpetas para que no se superponga con la recolección.", "Queues record migration so it cannot overlap collection.": "Encola la migración de registros para que no se superponga con la recolección.", "Pulls/starts (or refreshes) the changedetection + WAHA + webhook containers; data stays in var/integrations.": "Descarga/inicia (o actualiza) los contenedores de changedetection + WAHA + webhook; los datos permanecen en var/integrations.", "Writes container states plus the changedetection/WAHA URLs to the manual action log.": "Escribe los estados de los contenedores y las URLs de changedetection/WAHA en el registro de acciones manuales.", "Stops and starts the containers, applying the container settings saved below (changedetection URL, WAHA port/API key).": "Detiene e inicia los contenedores, aplicando la configuración de contenedores guardada abajo (URL de changedetection, puerto/clave API de WAHA).", "Stops and removes the changedetection/WAHA/webhook containers; their data stays in var/integrations.": "Detiene y elimina los contenedores de changedetection/WAHA/webhook; sus datos permanecen en var/integrations.", "Copies the selected template files into templates/ inside every saved record folder (existing files kept).": "Copia los archivos de plantilla seleccionados en templates/ dentro de cada carpeta de registro guardada (se conservan los archivos existentes).", "Queues saved-view rebuilding so it cannot overlap collection.": "Encola la reconstrucción de vistas guardadas para que no se superponga con la recolección.", "Queues deadline repair ahead of Cron and changedetection.": "Encola la reparación de fechas límite antes de Cron y changedetection.", "Re-downloads every record stuck at detail_status='failed', any group. Safe to click anytime: defers to priority 1/2/3 (skips cleanly and logs why if any is active) the same way the Closed backfill button does. See progress below.": "Vuelve a descargar cada registro atascado en detail_status='failed', de cualquier grupo. Seguro de usar en cualquier momento: se posterga a prioridad 1/2/3 (se omite limpiamente y registra por qué si alguna está activa) igual que el botón de relleno de Cerradas. Ver progreso abajo.", "Queues calendar packaging so it cannot overlap collection.": "Encola el empaquetado de calendarios para que no se superponga con la recolección.", "Queues calendar rebuilding and import so it cannot overlap collection.": "Encola la reconstrucción e importación de calendarios para que no se superponga con la recolección.", "Starts/restarts the local webhook listener.": "Inicia/reinicia el listener de webhook local.", "Installs/repairs the persistent user systemd webhook service.": "Instala/repara el servicio systemd de usuario persistente del webhook.", "Starts/opens the browser monitor (chromeless app window when available).": "Inicia/abre el monitor en el navegador (ventana de app sin marco cuando esté disponible).", "Opens the changedetection.io dashboard in a chromeless app window on the desktop — independent of Firefox, no browser header.": "Abre el panel de changedetection.io en una ventana de app sin marco en el escritorio — independiente de Firefox, sin encabezado de navegador.", "Writes the Browser Steps Execute JS instructions/script for Programadas + Abiertas pagination to the manual action log.": "Escribe las instrucciones/script de Execute JS de Browser Steps para la paginación de Programadas + Abiertas en el registro de acciones manuales.", "Opens the WAHA dashboard in a chromeless app window on the desktop (login: admin + the password from data/config/integration-access.txt).": "Abre el panel de WAHA en una ventana de app sin marco en el escritorio (inicio de sesión: admin + la contraseña de data/config/integration-access.txt).", 'Items matched': 'Ítems encontrados', 'Cotizaciones pricing': 'Precios de cotizaciones', 'Top items by quoted value': 'Ítems principales por valor cotizado', 'Most active providers': 'Proveedores más activos', 'Bids collected': 'Cotizaciones recolectadas', 'Items priced': 'Ítems con precio', 'Opportunities priced': 'Oportunidades con precio', 'Best-price total': 'Total al mejor precio', 'Avg-price total': 'Total a precio promedio', 'Max-price total': 'Total al precio máximo', 'Potential savings': 'Ahorro potencial', 'Avg price spread': 'Dispersión de precio promedio',
    ...
    'Collected opportunities by day, week, month or year. Click a month to open it, a day to zoom to its week, a week-day header to zoom to that day.': 'Oportunidades recolectadas por día, semana, mes o año. Haz clic en un mes para abrirlo, en un día para ampliar su semana, en el encabezado de un día de la semana para ampliar ese día.', 'Previous period': 'Período anterior', 'Jump to today': 'Ir a hoy', 'Next period': 'Período siguiente', '◀ Prev': '◀ Anterior', 'Next ▶': 'Siguiente ▶', 'Filters numero, descripcion, entidad, dependencia, modalidad and grupo. Accents ignored; comma = OR, + = AND, leading - = NOT.': 'Filtra numero, descripcion, entidad, dependencia, modalidad y grupo. Acentos ignorados; coma = OR, + = AND, - al inicio = NOT.', 'Filters only location facts: provincia, lugar, dirección, unidad de compra, dependencia, entidad. Same operators: comma = OR, + = AND, leading - = NOT.': 'Filtra solo datos de ubicación: provincia, lugar, dirección, unidad de compra, dependencia, entidad. Mismos operadores: coma = OR, + = AND, - al inicio = NOT.', 'Day': 'Día', 'Week': 'Semana', 'Month': 'Mes', 'Year': 'Año',
...
const PROCESS_PILL_LABELS = {{
  ...
  request: 'Request', closed_new: 'Closed: new closures', closed_backfill: 'Closed: backfill',
}};
...
async function applyClosedNewsSamplePages() {{
  const input = document.getElementById('news-sample-pages');
  const status = document.getElementById('closed-news-pages-status');
  const pages = input ? input.value.trim() : '';
  if (!pages) {{ if (status) status.textContent = 'Enter a page count first.'; return; }}
  if (!confirm(`Apply ${{pages}} sample pages? This restarts the changedetection container (a few seconds).`)) return;
  if (status) status.textContent = 'Applying…';
  try {{
    const res = await fetch('/api/closed-news-sample-pages', {{
      method: 'POST', headers: {{'Content-Type': 'application/x-www-form-urlencoded'}},
      body: 'pages=' + encodeURIComponent(pages),
    }});
    const result = await res.json();
    if (status) status.textContent = result.ok
      ? `Applied: watch now samples ${{result.pages}} page(s).`
      : `Failed: ${{result.error || 'unknown error'}}`;
  }} catch (err) {{ if (status) status.textContent = 'Failed: ' + err; }}
}}
async function loadClosedWebhookAccess() {{
  const node = document.getElementById('closed-webhook-access');
  if (!node) return;
  try {{
    const w = await (await fetch('/api/closed-webhook-access', {{cache: 'no-store'}})).json();
    node.textContent =
      ...
      `Local test from this machine:\n  ${{w.local_url}}\n\n` +
      `Generated by: ${{w.generated_by}}`;
  }} catch (err) {{ node.textContent = 'Closed webhook access unavailable: ' + err; }}
}}
async function loadChangedetectionScriptClosed() {{
  const box = document.getElementById('changedetection-script-closed');
  const state = document.getElementById('cd-script-closed-state');
  if (!box || (box.value && box.dataset.loaded === '1')) return;
  try {{
    const data = await (await fetch('/api/changedetection-script-closed', {{cache: 'no-store'}})).text();
    box.value = data;
    box.dataset.loaded = '1';
    if (state) state.textContent = 'Loaded from config/changedetection-browser-steps-closed.js';
  }} catch (err) {{
    if (state) state.textContent = 'Could not load script: ' + err;
  }}
}}
async function copyChangedetectionScriptClosed() {{
  await loadChangedetectionScriptClosed();
  const box = document.getElementById('changedetection-script-closed');
  const state = document.getElementById('cd-script-closed-state');
  if (!box) return;
  box.select();
...
async function loadClosedStatus() {{
  const pills = document.getElementById('closed-process-pills');
  ...
  try {{
    ...
    const npEl = document.getElementById('news-sample-pages');
    if (npEl && document.activeElement !== npEl && s.news_sample_pages !== undefined) npEl.value = s.news_sample_pages;
...
async function loadRepairStatus() {{
  const node = document.getElementById('repair-status');
  if (!node) return;
  try {{
    const s = await (await fetch('/api/repair-status', {{cache: 'no-store'}})).json();
    const p = s.progress || {{}};
    if (!Object.keys(p).length && !s.requested && !s.in_progress) {{
      node.textContent = t('(no repair run yet)');
      return;
    }}
    node.textContent =
      `${{t('Live process')}}: ${{s.running ? t('RUNNING') : t('idle')}}${{s.requested ? ' (' + t('queued') + ')' : ''}}\n` +
      `${{t('Phase')}}: ${{p.PHASE || '-'}}  ${{t('Status')}}: ${{p.STATUS || '-'}}  ${{p.PERCENT || 0}}%\n` +
      `${{t('Records total')}}: ${{p.RECORDS_TOTAL ?? '-'}}   ${{t('Processed')}}: ${{p.RECORDS_PROCESSED ?? '-'}}   ` +
      `${{t('Fixed')}}: ${{p.RECORDS_FIXED ?? '-'}}   ${{t('Still failed')}}: ${{p.RECORDS_STILL_FAILED ?? '-'}}\n` +
      `${{t('Started')}}: ${{p.STARTED_AT || '-'}}   ${{t('Updated')}}: ${{p.UPDATED_AT || '-'}}`;
  }} catch (err) {{ node.textContent = t('Repair status unavailable: ') + err; }}
}}
...
async function refreshDecisionDashboard() {{
  ...
  try {{
    ...
    if (cqNode) {{
      cqNode.innerHTML = [
        [t('Bids collected'), cq.total_bids || 0],
        [t('Items priced'), cq.total_items || 0],
        [t('Opportunities priced'), cq.opportunities_with_prices || 0],
        [t('Best-price total'), fmtMoney(cq.total_best_value)],
        [t('Avg-price total'), fmtMoney(cq.total_avg_value)],
        [t('Max-price total'), fmtMoney(cq.total_max_value)],
        [t('Potential savings'), fmtMoney(cq.potential_savings)],
...
function renderCotizacionesResults(rows) {{
  const node = document.getElementById('cotizaciones-results');
  if (!rows.length) {{ node.textContent = t('No quotations matched — try a different opportunity number or item keyword.'); return; }}
  const head = ['Opportunity', 'Item', 'Bidders', 'Min price', 'Avg price', 'Max price', 'Best provider', ''].map(h => `<th>${{esc(t(h))}}</th>`).join('');
  const body = rows.map(r => `<tr>` +
    `<td><code>${{esc(r.numero)}}</code></td>` +
    `<td>${{esc((r.item_descripcion || '').slice(0, 90))}}</td>` +
    `<td>${{r.bidder_count}}</td>` +
    `<td>${{fmtMoney(r.min_price)}}</td>` +
    `<td>${{fmtMoney(r.avg_price)}}</td>` +
    `<td>${{fmtMoney(r.max_price)}}</td>` +
    `<td>${{esc(r.best_proponente || '—')}}</td>` +
    `<td><button onclick="viewCotizacionDetail('${{encodeURIComponent(r.numero)}}')">${{esc(t('View full cuadro'))}}</button></td>` +
    `</tr>`).join('');
  node.innerHTML = `<table class="data-table"><thead><tr>${{head}}</tr></thead><tbody>${{body}}</tbody></table>`;
}}

async function viewCotizacionDetail(numeroEncoded) {{
  const numero = decodeURIComponent(numeroEncoded);
  const node = document.getElementById('cotizaciones-detail');
  node.innerHTML = `<p class="small">${{esc(t('Loading full cuadro…'))}}</p>`;
  try {{
    const data = await (await fetch('/api/cotizaciones-detail?numero=' + encodeURIComponent(numero), {{cache: 'no-store'}})).json();
    renderCotizacionDetail(numero, data.opportunity || null, data.rows || []);
  }} catch (err) {{ node.innerHTML = `<p class="small">${{esc(t('Could not load the full cuadro: '))}}${{esc(String(err))}}</p>`; }}
}}

function renderCotizacionDetail(numero, opportunity, rows) {{
  const node = document.getElementById('cotizaciones-detail');
  if (!rows.length) {{ node.innerHTML = `<p class="small">${{esc(t('No bid detail stored for this opportunity.'))}}</p>`; return; }}
  const byItem = new Map();
  rows.forEach(r => {{
    if (!byItem.has(r.item_index)) byItem.set(r.item_index, {{descripcion: r.item_descripcion, bids: []}});
    byItem.get(r.item_index).bids.push(r);
  }});
  const header = opportunity
    ? `<p class="small"><b>${{esc(opportunity.entidad || '')}}</b> — ${{esc((opportunity.descripcion || '').slice(0, 160))}}</p>`
    : '';
  const sections = Array.from(byItem.entries()).sort((a, b) => a[0] - b[0]).map(([itemIndex, item]) => {{
    const bidRows = item.bids.slice().sort((a, b) => (a.precio_unitario ?? Infinity) - (b.precio_unitario ?? Infinity)).map((b, i) => `<tr${{i === 0 ? ' style="font-weight:700"' : ''}}>` +
      `<td>${{esc(b.proponente)}}</td>` +
      `<td>${{fmtMoney(b.precio_unitario)}}</td>` +
      `<td>${{esc(b.cantidad_cotizada || '—')}}</td>` +
      `<td>${{fmtMoney(b.monto_neto)}}</td>` +
      `</tr>`).join('');
    return `<h4 class="small">#${{itemIndex}} — ${{esc((item.descripcion || '').slice(0, 140))}}</h4>` +
...
loadRepairStatus();
setInterval(loadRepairStatus, 20000);
refreshWebTimer();
poll();
{_LOC_TOOLTIP_JS}
</script>
...
class MonitorHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802 - stdlib handler API
        ...
        if path == "/api/cron-schedule":
            ...
            return
        if path == "/api/closed-news-sample-pages":
            pages = form.get("pages", [""])[0].strip()
            script = str(BASE_DIR / "src/50_tools/135-apply-closed-news-pages.py")
            result = subprocess.run(
                [script, pages], cwd=BASE_DIR, env=monitor_env(),
                capture_output=True, text=True, timeout=45,
            )
            out = (result.stdout.strip() or result.stderr.strip() or '{"ok": false, "error": "no output"}')
            self.send_text(200, out + "\n", "application/json; charset=utf-8")
            return
    ...
    def do_GET(self) -> None:  # noqa: N802 - stdlib handler API
        ...
        if path == "/api/cotizaciones":
            # Per-item price/provider comparison across every closed
            ...
            try:
                limit = max(1, min(1000, int(params.get("limit", ["200"])[0] or 200)))
            except (TypeError, ValueError):
                limit = 200
            try:
                conn = sqlite3.connect(f"file:{ARCHIVE_DB}?mode=ro", uri=True, timeout=2)
        ...
        if path == "/api/cotizacion-kpis":
            ...
            self.send_text(200, json.dumps(payload, ensure_ascii=False), "application/json; charset=utf-8")
            return
        if path == "/api/cotizaciones-detail":
            # Full bidder-by-bidder breakdown for one closed opportunity —
            # what "View full cuadro" drills into from a /api/cotizaciones row.
            params = parse_qs(urlparse(self.path).query)
            numero = params.get("numero", [""])[0].strip()
            if not numero:
                self.send_text(400, json.dumps({"error": "numero required"}, ensure_ascii=False), "application/json; charset=utf-8")
                return
            try:
                conn = sqlite3.connect(f"file:{ARCHIVE_DB}?mode=ro", uri=True, timeout=2)
                conn.row_factory = sqlite3.Row
                try:
                    rows = pc_common.cotizacion_bids_for_numero(conn, numero)
                    opp_row = conn.execute(
                        "SELECT numero, descripcion, entidad, link FROM opportunities WHERE numero = ?",
                        (numero,),
                    ).fetchone()
                finally:
                    conn.close()
                ...
                rows = []
                opp_row = None
            payload = {"rows": rows, "opportunity": dict(opp_row) if opp_row else None}
            self.send_text(200, json.dumps(payload, ensure_ascii=False), "application/json; charset=utf-8")
            return
        ...
        if path == "/api/changedetection-script":
            ...
            return
        if path == "/api/closed-webhook-access":
            self.send_text(200, json.dumps(closed_webhook_access_payload(), ensure_ascii=False), "application/json; charset=utf-8")
            return
        if path == "/api/changedetection-script-closed":
            try:
                script_text = CHANGEDETECTION_BROWSER_STEPS_CLOSED_JS.read_text(encoding="utf-8")
            except OSError as exc:
                self.send_text(404, f"Closed changedetection script not found: {exc}\n", "text/plain; charset=utf-8")
                return
            self.send_text(200, script_text, "text/javascript; charset=utf-8")
            return
        ...
        if path == "/api/task-queue":
            ...
            return
        if path == "/api/repair-status":
            self.send_text(200, json.dumps(repair_status_payload(), ensure_ascii=False), "application/json; charset=utf-8")
            return
