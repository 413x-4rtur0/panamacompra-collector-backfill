...
VALUE_SETTING_DEFAULTS = {
    ...
    # default via PC_CERRADAS_BACKFILL_DAYS). See 037-collect-cerradas-index.py.
    "PC_CERRADAS_BACKFILL_START_DATE": "",
    "PC_CERRADAS_BACKFILL_END_DATE": "",
...
<div class="card" data-tab="integrations"><h2>Cerradas monitor <span class="small">(second, independent process)</span></h2><p class="small">Closed-opportunity crawl runs completely separately from the Abiertas/Programadas pipeline above — its own changedetection watch, its own webhook token, its own systemd timer for the historical backfill — so nothing here can block or crash that one. Priority 2 (new closures) reacts to its changedetection watch; priority 3 (backfill) ticks on a timer and defers to both priority 1 and 2. Neither sends WhatsApp; both only download and insert into the database.</p><div id="cerradas-process-pills" class="small">Loading…</div><pre id="cerradas-status">Loading Cerradas status…</pre><p><button onclick="loadCerradasStatus()">Refresh Cerradas status</button></p><div class="subsection"><h3>Backfill date range (optional)</h3><p class="small">Leave both blank for the default day-count backfill (PC_CERRADAS_BACKFILL_DAYS, currently controls how far back it goes). Set a range instead to target specific history: Start date replaces the day-count cutoff outright; End date makes the backfill skip newer rows until paging reaches it. Reset the backfill cursor first if you want a fresh range instead of continuing from where it left off.</p><div class="settings-grid"><label class="small">Start date <input id="set-PC_CERRADAS_BACKFILL_START_DATE" type="date"></label> <label class="small">End date <input id="set-PC_CERRADAS_BACKFILL_END_DATE" type="date"></label> <button onclick="saveCerradasDateRange()">Save date range</button></div></div></div>
...
async function saveCerradasDateRange() {{
  if (!monitorSettingsLoaded) {{ alert('Settings are still loading — try again in a moment.'); return; }}
  ...
  for (const key of ['PC_CERRADAS_BACKFILL_START_DATE', 'PC_CERRADAS_BACKFILL_END_DATE']) {{
