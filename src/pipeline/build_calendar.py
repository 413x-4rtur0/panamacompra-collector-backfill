...
from common import CALENDAR_DIR, COMBINED_CALENDAR_PATH, RECORDS_DIR, calendars_to_ics, date_folder_name, write_run_progress
...
def main() -> int:
    ...
    write_run_progress("CALENDAR", "RUNNING", 96, "Step 6/6: collecting new calendar events for ICS packages...", step_current=6, step_total=6, extra=f"package_size={package_size}")
    ...
    write_run_progress("CALENDAR", "DONE", 98, f"Step 6/6 complete. Wrote {len(written)} calendar package(s) with {count} new event(s).", step_current=6, step_total=6, item_current=count, item_total=count, extra=f"out_dir={out_dir}")
    return 0
