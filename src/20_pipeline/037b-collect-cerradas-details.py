...
background Cerradas discovery never floods the priority-1 detail queue (see
that file's own comment on detail_pending_rows()). This script gives
Cerradas records that same full archive, on its own low-resource,
non-competing schedule -- called alongside 037/038 from the Cerradas-only
orchestrators (priority 2 new-closures, priority 3 backfill), never
priority 1.

Queue: Cerradas rows with detail_attempts under the limit and either no
...
completeness check), just scoped to grupo='Cerradas' instead of excluding
...
does, via a separate call this script never makes), since Cerradas stays
...
def cerradas_fulldetail_limit() -> int:
    return env_int("PC_CERRADAS_FULLDETAIL_LIMIT", "10", minimum=1)


def cerradas_fulldetail_max_attempts() -> int:
    return env_int("PC_CERRADAS_FULLDETAIL_MAX_ATTEMPTS", "3", minimum=1)


def cerradas_detail_pending_rows(conn, limit: int, max_attempts: int):
    """Same shape/gate as 030-collect-details.py's detail_pending_rows(),
    scoped to grupo='Cerradas' instead of excluding it."""
...
def main():
    conn = init_db()
    limit = cerradas_fulldetail_limit()
    max_attempts = cerradas_fulldetail_max_attempts()
    rows = cerradas_detail_pending_rows(conn, limit, max_attempts)
    run_started = now_iso()

    if not rows:
        print("No Cerradas records pending a full detail fetch.")
        return
    ...
    summary = (
        f"CERRADAS FULL-DETAIL RUN started: {run_started}\n"
        f"CERRADAS FULL-DETAIL RUN finished: {now_iso()}\n"
        f"Queue size this run: {len(rows)} (limit={limit})\n"
        f"Saved: {saved}  Skipped/complete: {skipped}  Failed: {failed}\n"
    )
    log_path = LOG_DIR / f"cerradas_fulldetail_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
