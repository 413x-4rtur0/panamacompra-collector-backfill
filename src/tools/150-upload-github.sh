...
usage() {
  cat <<'USAGE'
Usage: src/tools/150-upload-github.sh [--message TEXT] [--remote NAME] [--branch NAME] [--dry-run] [--run-checks] [--allow-empty]
...
Notes:
  ...
  - Configure credentials with src/tools/120-setup-git-credentials.sh when using
...
if [ "$RUN_CHECKS" = "1" ]; then
  ...
  python3 -m py_compile src/notify/010-waha-client.py src/monitor/001a-monitor-tk.py src/monitor/001b-monitor-web.py src/pipeline/020-notify-whatsapp.py src/tools/040-message-formats.py src/tools/140-full-report.py
  bash -n bin/pcc review-system.sh src/pipeline/100-run-worker.sh src/tools/010-docker-stack.sh
  git diff --check
fi
