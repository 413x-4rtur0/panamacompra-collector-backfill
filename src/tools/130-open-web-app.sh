...
#
#   ./src/tools/130-open-web-app.sh monitor           web monitor (starts the server if needed)
#   ./src/tools/130-open-web-app.sh changedetection   changedetection.io dashboard
#   ./src/tools/130-open-web-app.sh waha              WAHA dashboard (login: see integration-access.txt)
#   ./src/tools/130-open-web-app.sh http://host:port/ any URL
#
...
# src/tools/010-docker-stack.sh) so the app opens the same URLs the stack uses.
...
ensure_web_monitor() {
  ...
  PC_MONITOR_HOST="$MONITOR_HOST" PC_MONITOR_PORT="$MONITOR_PORT" \
    nohup "$PYTHON_BIN" "$APP_ROOT/src/monitor/001b-monitor-web.py" >> "$PC_LOG_DIR/monitor_server.log" 2>&1 &
  sleep 1
}
