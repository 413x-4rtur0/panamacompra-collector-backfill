...
async function refreshDecisionDashboard() {{
  ...
  k.innerHTML = [ ['Total opportunities', s.total || 0], ['Completed', s.saved || 0], ['Pending', s.pending || 0], ['Failed', s.failed || 0], ['Needs deadline repair', s.needs_deadline || 0], ['Closure rate', closure + '%'] ].map(x => `<div class="kpi"><span>${{x[0]}}</span><b>${{x[1]}}</b></div>`).join('');
  ...
  const words = ['run','stop','pending','manual','test','notify','WAHA','deadline','region','trend','repair','calendar','templates','webhook'];
  ...
  document.getElementById('decision-recommendations').textContent = `Decision signals\n• Prioritize deadline repair when Needs deadline repair > 0; missing DTEND weakens calendar and expiry decisions.\n• Use Groups / regions / buyers to spot concentrated demand and decide monitoring keywords.\n• Watch Pending + Failed together: high values mean collector capacity or portal stability needs action.\n• Use Notify backlog to confirm WAHA alerts are supporting decisions, not just collecting data.`;
}}
