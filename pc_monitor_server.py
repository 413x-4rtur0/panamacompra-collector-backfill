...
.form-grid {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 10px 14px; align-items: end; }}
...
.subsection {{ border-top: 1px solid #334155; margin-top: 14px; padding-top: 12px; }}
.subsection h3 {{ margin: 0 0 8px; color: #fef3c7; }}
</style>
</head>
<body>
<div class="card">
...
<div class="card"><h2>Monitor controls</h2>
  <div class="subsection"><h3>Run request</h3>
    <div class="form-grid">
      <label><span class="small">Mode</span><span class="mode-group" id="run-mode"><label><input type="radio" name="run-mode" value="auto" disabled><span>automatic</span></label><label><input type="radio" name="run-mode" value="restart" checked><span>restart pending</span></label><label><input type="radio" name="run-mode" value="manual"><span>manual run</span></label><label><input type="radio" name="run-mode" value="test"><span>test run</span></label></span></label>
      <label class="small limit-control">Index limit <input id="index-limit" value="20" size="4"></label>
      <label class="small limit-control">Detail limit <input id="detail-limit" value="99" size="4"></label>
      <label class="small limit-control">Start from <select id="detail-order"><option value="newest">newest DB record</option><option value="oldest" selected>oldest DB record</option></select></label>
    </div>
    <div class="button-row"><button id="run-button" class="primary" onclick="requestRun()">Request selected run</button><button class="danger" onclick="stopRun()">Stop active run</button><span id="button-status" class="small"></span></div>
    <p class="small" id="run-hint"><strong>Mode:</strong> automatic is shown for changedetection/webhook runs only; restart pending queues the normal collector; manual run starts the worker now; test run uses the isolated test zone. Automatic/restart pending use normal defaults. Index/detail limits and newest/oldest start order are only enabled for manual and test runs.</p>
  </div>
  <div class="subsection"><h3>WhatsApp and calendar</h3>
    <textarea id="waha-message" placeholder="WhatsApp group/channel chat ID destination"></textarea>
    <div class="button-row"><button onclick="saveWaha()">Save WhatsApp destination</button><label class="small"><input type="checkbox" id="notify-whatsapp" onchange="saveMonitorSetting('PC_NOTIFY_WHATSAPP', this.checked ? '1' : '0')"> Notify by WhatsApp after detail/calendar</label><label class="small"><input type="checkbox" id="calendar-auto-import" onchange="saveMonitorSetting('PC_CALENDAR_AUTO_IMPORT', this.checked ? '1' : '0')"> Import/open generated calendar events</label><label class="small">Default order <select id="setting-detail-order" onchange="saveMonitorSetting('PC_DETAIL_ORDER', this.value)"><option value="oldest">oldest pending first</option><option value="newest">newest pending first</option></select></label></div>
  </div>
  <div class="subsection"><h3>Advanced settings</h3>
    <div class="form-grid"><label class="small">WAHA source <input id="waha-source" size="16"></label><label class="small">Soon days <input id="soon-days" size="4"></label><label class="small">Max detail attempts <input id="max-detail-attempts" size="4"></label><label class="small">Active refresh <input id="web-refresh" size="4"></label><label class="small">Idle refresh <input id="web-idle-refresh" size="4"></label><label class="small">Auto-close <input id="web-auto-close" size="4"></label></div>
    <div class="button-row"><button onclick="saveAdvancedSettings()">Save advanced</button></div>
  </div>
  <div class="subsection"><h3>Folders</h3>
    <div class="form-grid"><label class="small">Records folder <input id="records-dir" size="42"></label><label class="small">Calendar packages <input id="calendar-dir" size="42"></label><label class="small">Test sandbox <input id="records-test-dir" size="42"></label><label class="small">Index folder <input id="index-dir" size="42"></label><label class="small">Config folder <input id="config-dir" size="42"></label></div>
    <div class="button-row"><button onclick="savePathSettings()">Save paths</button><button onclick="openSettingFolder('PC_RECORDS_DIR')">Open records</button><button onclick="openSettingFolder('PC_CALENDAR_DIR')">Open calendar</button><button onclick="openSettingFolder('PC_RECORDS_TEST_DIR')">Open test</button><button onclick="openSettingFolder('PC_INDEX_DIR')">Open index</button><button onclick="openSettingFolder('PC_CONFIG_DIR')">Open config</button></div>
  </div>
  <div class="subsection"><h3>Script actions</h3><div id="action-zones"></div></div>
</div>
<div class="card"><h2>Diagnostics</h2><table id="diagnostics"></table></div>
<div class="card"><h2>Record index</h2>
  <p class="small">Collected records as “[downloaded timestamp | DTEND status] NUMERO — description”, sorted by DTEND (soonest deadline first). Ctrl/Shift-select one or more records to notify or import calendars.</p>
  <div class="subsection"><h3>Filters</h3>
    <div class="form-grid"><label class="small">Status <select id="record-status"><option value="all">All</option><option value="soon">Next to expire</option><option value="expired">Expired</option><option value="upcoming">Upcoming</option></select></label><label class="small">DTEND on/after <input type="date" id="record-mindate"></label><label class="small">Detail saved on/after <input type="date" id="record-downloaded-mindate"></label><label class="small">Index inserted on/after <input type="date" id="record-indexed-mindate"></label><label class="small">Last seen on/after <input type="date" id="record-lastseen-mindate"></label><label class="small">Status changed on/after <input type="date" id="record-statuschanged-mindate"></label><label class="small">Folder renamed on/after <input type="date" id="record-renamed-mindate"></label><label class="small">Notified on/after <input type="date" id="record-notified-mindate"></label></div>
    <p class="small">Legend: <span style="color:#86efac;font-weight:700">upcoming</span> · <span style="color:#fcd34d;font-weight:700">next to expire</span> · <span style="color:#fca5a5;font-weight:700">expired</span></p>
  </div>
  <div class="subsection"><h3>Records and actions</h3>
    <select id="record-index" multiple size="10"></select>
    <div class="button-row"><button onclick="refreshRecordIndex()">Refresh list</button><button onclick="openRecordFolder()">Open record folder</button><button onclick="openDetailFolder()">Open details folder</button><button onclick="openRecordPortal()">Open in portal</button><button onclick="notifySelectedRecords()">Notify selected WhatsApp</button><button onclick="importSelectedCalendars()">Import selected calendars</button></div>
    <p id="record-detail" class="small">Loading record index…</p>
  </div>
</div>
...
function updateRunControls(data) {{
  ...
  document.querySelectorAll('.limit-control').forEach(el => {{ el.style.display = limitsAllowed ? 'inline-block' : 'none'; }});
  const hint = document.getElementById('run-hint');
  if (hint && selected && !busy) hint.textContent = ['manual', 'test'].includes(selected.value) ? 'Manual/test mode: optional positive limits and newest/oldest start order are enabled.' : 'Automatic/restart mode: index and detail run unlimited; use manual/test for capped checks.';
...
function savePathSettings() {{
  [['PC_RECORDS_DIR', 'records-dir'], ['PC_CALENDAR_DIR', 'calendar-dir'], ['PC_RECORDS_TEST_DIR', 'records-test-dir'], ['PC_INDEX_DIR', 'index-dir'], ['PC_CONFIG_DIR', 'config-dir']].forEach(([key, id]) => saveMonitorSetting(key, document.getElementById(id).value));
}}
...
function openDetailFolder() {{
  const rec = selectedRecord();
  if (!rec) {{ document.getElementById('button-status').textContent = 'Select a record first.'; return; }}
  postForm('/api/open-detail-folder', 'numero=' + encodeURIComponent(rec.numero));
}}
...
document.querySelectorAll('input[name="run-mode"]').forEach(el => el.addEventListener('change', () => updateRunControls({{processes: {{}}, progress: {{MODE: el.value.toUpperCase()}}}})));
renderActionZones();
...
class MonitorHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802 - stdlib handler API
        ...
        if path == "/api/open-setting-folder":
            ...
            return
        if path == "/api/open-detail-folder":
            numero = form.get("numero", [""])[0].strip()
            for record in load_record_index():
                if record["numero"] == numero:
                    detail_path = record.get("detail_json_path") or ""
                    folder = Path(detail_path).parent if detail_path else Path(record["record_folder"]) / "details"
                    if not folder.exists():
                        self.send_text(404, f"Details folder not found on disk for {numero}.\n", "text/plain; charset=utf-8")
                        return
                    open_folder(folder)
                    self.send_text(202, f"Opened details folder for {numero}.\n", "text/plain; charset=utf-8")
                    return
            self.send_text(404, f"Unknown record: {numero}\n", "text/plain; charset=utf-8")
            return
