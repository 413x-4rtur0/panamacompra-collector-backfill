...
function initCollapsibleSections() {{
  ...
  document.querySelectorAll('.card').forEach((card, idx) => {{
    ...
    if (idx === 0 || card.classList.contains('data-open') || ['Recent worker log', 'Current action log'].includes((heading && heading.textContent || '').trim())) return;
